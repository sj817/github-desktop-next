...
> {
    ...
    return (
      <Row className="merge-button-row">
        <Button
          ...
          tooltip={t('Choose a branch to merge into {{name}}', {
            name: currentBranch.name,
          })}
        >
          <Octicon className="icon" symbol={octicons.gitMerge} />
          <Trans i18nKey='branches.choose-branch-to-merge-into'>
            Choose a branch to merge into{' '}
            <strong>{{ name: currentBranch.name }}</strong>
          </Trans>
        </Button>
      </Row>
    )
    ...
    return (
      ...
      >
        ...
        <span id="pull-requests-tab" className="pull-request-tab">
          {t(__DARWIN__ ? 'Pull Requests' : 'Pull requests')}
  ...
  private renderPreList = () => {
    ...
    const label = t(__DARWIN__ ? 'New Branch' : 'New branch')
