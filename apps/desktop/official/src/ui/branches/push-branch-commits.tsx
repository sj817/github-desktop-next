...
> {
      ...
      return (
        <DialogContent>
          <p id="push-branch-commits-title">
            {t('Your branch must be published before opening a pull request.')}
          </p>
          <p id="push-branch-commits-message">
            <Trans i18nKey='push-branch-commits.publish-and-open-pr'>
  ...
  private renderDialogTitle() {
    if (renderPublishView(this.props.unPushedCommits)) {
      return t(__DARWIN__ ? 'Publish Branch?' : 'Publish branch?')
    }

    return t(__DARWIN__ ? 'Push Local Changes?' : 'Push local changes?')
  }
  ...
  private renderButtonGroup() {
    if (renderPublishView(this.props.unPushedCommits)) {
      return (
        <OkCancelButtonGroup
          okButtonText={t(__DARWIN__ ? 'Publish Branch' : 'Publish branch')}
        />
      )
    }
    ...
    return (
      <OkCancelButtonGroup
        okButtonText={t(__DARWIN__ ? 'Push Commits' : 'Push commits')}
        cancelButtonText={t(
