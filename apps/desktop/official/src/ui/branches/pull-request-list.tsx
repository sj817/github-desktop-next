...
> {
  ...
  public componentWillReceiveProps(nextProps: IPullRequestListProps) {
    ...
    const screenReaderStateMessage = loadingStarted
      ? t('Hang Tight. Loading pull requests as fast as I can!')
      : loadingComplete
      ? t(
          numPullRequests === 1
            ? '{{numPullRequests}} pull request found'
            : '{{numPullRequests}} pull requests found',
          { numPullRequests }
        )
      : null
  ...
  private getListAriaLabel = () => {
    return t('Pull requests in {{name}}', { name: this.getRepositoryName() })
  }
  ...
  private renderListHeader = () => {
    return (
      <div className="filter-list-group-header">
        {t('Pull requests in {{name}}', { name: this.getRepositoryName() })}
      </div>
    )
  }
  ...
  private renderPostFilter = () => {
    const tooltip = t('Refresh the list of pull requests')
