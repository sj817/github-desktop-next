...
> {
  ...
  private renderBranchSelection() {
    ...
    if (targetCommit !== undefined) {
      return (
        <p>
          {t(
            "Your new branch will be based on the commit '{{summary}}' ({{sha}}) from your repository.",
            {
      ...
      return (
        <p>
          {t(
            'You do not currently have any branch checked out (your HEAD reference is detached). As such your new branch will be based on your currently checked out commit ({{sha}}).',
  ...
  private getHeaderText = (): string => {
    ...
    return t(__DARWIN__ ? 'Create a Branch' : 'Create a branch')
  }
  ...
  private getOkButtonText = (): string => {
    ...
    return t(__DARWIN__ ? 'Create Branch' : 'Create branch')
  }
  ...
  private async updateBranchName(branchName: string) {
    ...
    const currentError = alreadyExists
      ? {
          error: new Error(t('A branch named {{branchName}} already exists.', { branchName })),
          isWarning: false,
        }
      : null
  ...
  private createBranch = async () => {
    ...
    if (this.props.targetCommit !== undefined) {
      ...
      if (!defaultBranch) {
        this.setState({
          currentError: {
            error: new Error(t('Could not determine the default branch.')),
            isWarning: false,
          },
        })
        return
  ...
  private renderRegularBranchSelection(
    ...
    if (defaultBranch === null || defaultBranch.name === currentBranchName) {
      return (
        <div>
          <Trans i18nKey='create-branch.based-on-current-branch'>
          ...
          {defaultBranch?.name === currentBranchName && (
            <Trans i18nKey='create-branch.current-is-default'>
  ...
  private renderForkBranchSelection(
    ...
    if (currentBranchName === upstreamDefaultBranch.nameWithoutRemote) {
      return (
        <div>
          <Trans i18nKey='create-branch.based-on-upstream-default'>
    ...
    if (isRepositoryWithForkedGitHubRepository(this.props.repository)) {
      return (
        <div className="secondary-text">
          <Trans i18nKey='create-branch.fork-link'>
  ...
  private renderForkLinkSuffix = () => {
    if (isRepositoryWithForkedGitHubRepository(this.props.repository)) {
      return (
        <span>
          <Trans i18nKey='create-branch.fork-link-suffix'>
  ...
  private renderOptions = (
    ...
    <Row>
      <VerticalSegmentedControl
        label={t('Create branch based on…')}
        items={items}
