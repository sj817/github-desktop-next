  ...
  private getWarningMessageForItem(item: DiffContentsWarningItem) {
    switch (item.type) {
      case DiffContentsWarningType.UnicodeBidiCharacters:
        return (
          <Trans i18nKey='diff-contents-warning.bidi-characters'>
      ...
      case DiffContentsWarningType.LineEndingsChange:
        ...
        return (
          <>
            {t(
              "This diff contains a change in line endings from '{{from}}' to '{{to}}'.",
              { from: lineEndingsChange.from, to: lineEndingsChange.to }
            )}
          </>
        )
    }
