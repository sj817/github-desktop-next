...
> {
  ...
  private onExpandHunk = (
    ...
    this.setState({ ariaLiveMessage: t('Expanded') })
  }
  ...
  private buildExpandMenuItem(): IMenuItem | null {
    ...
    return this.diffToRestore === null
      ? {
          label: t(__DARWIN__ ? 'Expand Whole File' : 'Expand whole file'),
      ...
      : {
          label: t(
            __DARWIN__ ? 'Collapse Expanded Lines' : 'Collapse expanded lines'
          ),
  ...
  private getDiscardLabel(rangeType: DiffRangeType, numLines: number): string {
    ...
    const multiple = numLines > 1

    if (rangeType === DiffRangeType.Additions) {
      return multiple
        ? t(__DARWIN__ ? 'Discard Added Lines{{suffix}}' : 'Discard added lines{{suffix}}', { suffix })
        : t(__DARWIN__ ? 'Discard Added Line{{suffix}}' : 'Discard added line{{suffix}}', { suffix })
    } else if (rangeType === DiffRangeType.Deletions) {
      return multiple
        ? t(__DARWIN__ ? 'Discard Removed Lines{{suffix}}' : 'Discard removed lines{{suffix}}', { suffix })
        : t(__DARWIN__ ? 'Discard Removed Line{{suffix}}' : 'Discard removed line{{suffix}}', { suffix })
    } else if (rangeType === DiffRangeType.Mixed) {
      return multiple
        ? t(__DARWIN__ ? 'Discard Modified Lines{{suffix}}' : 'Discard modified lines{{suffix}}', { suffix })
        : t(__DARWIN__ ? 'Discard Modified Line{{suffix}}' : 'Discard modified line{{suffix}}', { suffix })
    } else {
      return assertNever(rangeType, `Invalid range type: ${rangeType}`)
    }
  }
  ...
  private onSearch = (searchQuery: string, direction: SearchDirection) => {
    ...
    if (searchQuery?.trim() === '') {
      this.resetSearch(true, t('No results'))
  ...
  private startSearch = (searchQuery: string, direction: SearchDirection) => {
    ...
    if (searchResults === undefined || searchResults.length === 0) {
      ...
      const ariaLiveMessage = t(
        'Result 1 of {{count}} for "{{searchQuery}}"',
        { count: searchResults.length, searchQuery }
      )
  ...
  private continueSearch = (
    ...
    const ariaLiveMessage = t(
      'Result {{result}} of {{count}} for "{{searchQuery}}"',
      {
        result: selectedSearchResult + 1,
