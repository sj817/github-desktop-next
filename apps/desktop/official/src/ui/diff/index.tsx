    ...
    return (
      <div className="panel empty large-diff">
        ...
        <div className="description">
          <p>{t('The diff is too large to be displayed by default.')}</p>
          <p>
            {t(
        ...
        <Button onClick={this.showLargeDiff}>
          {t(__DARWIN__ ? 'Show Diff' : 'Show diff')}
        </Button>
      </div>
    )
  }
  ...
  private renderUnrenderableDiff() {
    return (
      <div className="panel empty large-diff">
        ...
        <p>{t('The diff is too large to be displayed.')}</p>
      </div>
    )
  }
  ...
  private renderText(diff: ITextDiff) {
    if (diff.hunks.length === 0) {
      if (
        ...
        return <div className="panel empty">{t('The file is empty')}</div>
      }
      ...
      if (this.props.file.status.kind === AppFileStatusKind.Renamed) {
        ...
        if (this.props.file.status.renameIncludesModifications) {
          return (
            <div className="panel renamed">
              ...
              <Trans i18nKey='diff.renamed-and-changed'>
        ...
        return (
          <div className="panel renamed">
            {t('The file was renamed but not changed')}
          </div>
        )
      }
      ...
      if (this.props.hideWhitespaceInDiff) {
        return (
          <div className="panel empty">{t('Only whitespace changes found')}</div>
        )
      }

      return (
        <div className="panel empty">{t('No content changes found')}</div>
      )
    }
