...
interface ICommitMessageAvatarProps {
  ...
  /**
   * Switch the active account. When provided and more than one account is
   * signed in, the avatar popover offers a quick account switcher. The first
   * account in `accounts` is the active one.
   */
...
> {
  ...
  public render() {
    ...
    switch (warningType) {
      case 'none':
        ariaLabel = t('View commit author information')
        break

      case 'misattribution':
        ariaLabel = t('Commit may be misattributed. View warning.')
        break

      case 'disallowedEmail':
        ariaLabel = t('Email address is disallowed. View warning.')
        break
    }
  ...
  private renderAccountSwitcher() {
    const { accounts, onSetActiveAccount } = this.props
    if (onSetActiveAccount === undefined || accounts.length < 2) {
      return null
    }

    return (
      <div className="commit-avatar-account-switcher">
        <h4>{t('Switch account')}</h4>
        {accounts.map((account, index) => (
          <button
            key={`${account.endpoint}|${account.id}`}
            type="button"
            className={`commit-avatar-account${
              index === 0 ? ' is-active' : ''
            }`}
            onClick={() => this.onSwitchAccount(account)}
            disabled={index === 0}
          >
            <img src={account.avatarURL} alt="" width={20} height={20} />
            <span className="commit-avatar-account-login">{account.login}</span>
            {index === 0 && <Octicon symbol={octicons.check} />}
          </button>
        ))}
      </div>
    )
  ...
  private renderGitConfigPopover() {
    ...
    const location = t(isGitConfigLocal ? 'local' : 'global')
    const locationDesc = isGitConfigLocal ? t('for your repository') : ''
    const settingsName = t(__DARWIN__ ? 'settings' : 'options')
    const settings = isGitConfigLocal
      ? t('repository settings')
      : t('git {{settingsName}}', { settingsName })
    const buttonText = t(
    ...
    return (
      <>
        <p>
          {user &&
            user.name &&
            t('Email: {{email}}', { email: user.email })}
        </p>
        ...
        {!isGitConfigLocal && (
          <p className="secondary-text">
            <Trans i18nKey='commit-message-avatar.set-email-local'>
  ...
  private renderWarningPopover() {
    ...
    const updateEmailTitle = t(__DARWIN__ ? 'Update Email' : 'Update email')

    const sharedHeader = (
      <Trans i18nKey='commit-message-avatar.shared-header'>
        The email in your global Git config (
        <span className="git-email">{{ email: this.props.email }}</span>)
      </Trans>
    )
      ...
      <>
        ...
        <Row>
          <div className="secondary-text">
            <Trans i18nKey='commit-message-avatar.choose-email-local'>
              You can{{ also: hasEmails ? t(' also') : '' }} choose an email
              local to this repository from the{' '}
              <LinkButton onClick={this.onRepositorySettingsClick}>
                repository settings
              </LinkButton>
              .
    ...
    if (warningType === 'misattribution') {
      ...
      const userName =
        this.props.user && this.props.user.name
          ? t(' for {{name}}', { name: this.props.user.name })
          : ''
      ...
      return (
        <>
          <Row>
            <div>
              <Trans i18nKey='commit-message-avatar.misattribution'>
                {sharedHeader} doesn't match your GitHub{{ accountTypeSuffix }}{' '}
                account{{ userName }}.{' '}
                <LinkButton
                  ariaLabel={t('Learn more about commit attribution')}
  ...
  private getCommittingAsTitle(): string | JSX.Element | undefined {
    ...
    if (name) {
      return (
        <Trans i18nKey='commit-message-avatar.committing-as'>
          Committing as <strong>{{ name }}</strong>
        </Trans>
      )
    }

    return t('Committing with {{email}}', { email })
  }
  ...
  private renderPopover() {
    ...
    switch (this.props.warningType) {
      case 'misattribution':
        header = t('This commit will be misattributed')
        break

      case 'disallowedEmail':
        header = t('This email address is disallowed')
        break
