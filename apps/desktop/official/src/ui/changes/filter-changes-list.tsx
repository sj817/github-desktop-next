...
> {
  ...
  private getDiscardChangesMenuItemLabel = (files: ReadonlyArray<string>) => {
    const label =
      files.length === 1
        ? t(__DARWIN__ ? 'Discard Changes' : 'Discard changes')
        : t(
            __DARWIN__
  ...
  private onContextMenu = (event: React.MouseEvent<any>) => {
    ...
    const stashAllChangesLabel = t(
      __DARWIN__ ? 'Stash All Changes' : 'Stash all changes'
    )
    const confirmStashAllChangesLabel = t(
      __DARWIN__ ? 'Stash All Changes…' : 'Stash all changes…'
    )
  ...
  private getOpenInExternalEditorMenuItem = (
    ...
    const openInExternalEditor = externalEditorLabel
      ? t('Open in {{externalEditorLabel}}', { externalEditorLabel })
      : DefaultEditorLabel()
  ...
  private getDefaultContextMenu(
    ...
    if (paths.length === 1) {
      ...
      items.push({
        label: t(
          __DARWIN__
            ? 'Ignore {{count}} Selected Files (Add to .gitignore)'
            : 'Ignore {{count}} selected files (add to .gitignore)',
          { count: paths.length }
        ),
        action: () => {
    ...
    if (paths.length > 1) {
      items.push(
        ...
        {
          label: t(
            __DARWIN__ ? 'Include Selected Files' : 'Include selected files'
          ),
          action: () => {
        ...
        {
          label: t(
            __DARWIN__ ? 'Exclude Selected Files' : 'Exclude selected files'
          ),
          action: () => {
  ...
  private getPlaceholderMessage(
    ...
    switch (firstFile.status.kind) {
      ...
      case AppFileStatusKind.Untracked:
        return t('Create {{fileName}}', { fileName })
      case AppFileStatusKind.Deleted:
        return t('Delete {{fileName}}', { fileName })
      default:
        // TODO:
        ...
        return t('Update {{fileName}}', { fileName })
    }
  }
  ...
  private renderStashedChanges() {
    ...
    return (
      ...
      >
        ...
        <div className="text">{t('Stashed Changes')}</div>
  ...
  private renderCheckBoxRow = () => {
    ...
    const changedFilesLabel = t('{{formattedCount}} changed file_filter', {
      count: files.length,
      formattedCount: formatNumber(files.length),
    })
    const checkAllLabel =
      visibleFiles !== files.length
        ? t('{{visibleCount}} of {{changedFilesLabel}}', {
            visibleCount: formatNumber(visibleFiles),
            changedFilesLabel,
          })
        : changedFilesLabel
  ...
  private getListAriaLabel = () => {
    ...
    return t('{{formattedCount}} changed file_filter', {
      count: files.length,
      formattedCount: formatNumber(files.length),
    })
  }
  ...
  private renderHiddenChangesWarning = () => {
    ...
    return (
      <div className="hidden-changes-warning" id="hidden-changes-warning">
        ...
        <span>{t('Hidden changes will be committed. ')}</span>
        <LinkButton onClick={this.showFilesToBeCommitted}>
          <Trans i18nKey='filter-changes-list.adjust-filters-to-see-all'>
            Adjust the filters to see all{' '}
            {{ count: formatNumber(filesSelected.length) }} changes
          </Trans>
        </LinkButton>
      </div>
    )
  ...
  private renderNoChanges = () => {
    ...
    return (
      <div className="no-changes-filtered">
        ...
        <div className="title">{t('No files match your current filters')}</div>
