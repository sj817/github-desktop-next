...
interface ICommitMessageProps {
  ...
  /** Switch the active account from the commit avatar's quick switcher. */
...
> {
  ...
  private renderAvatar() {
    ...
    return (
      <CommitMessageAvatar
        ...
        onSetActiveAccount={this.props.onSetActiveAccount}
      />
    )
  }
  ...
  private get toggleCoAuthorsText(): string {
    return this.props.showCoAuthoredBy
      ? t(__DARWIN__ ? 'Remove Co-Authors' : 'Remove co-authors')
      : t(__DARWIN__ ? 'Add Co-Authors' : 'Add co-authors')
  }
  ...
  private getCommitSpellcheckEnabilityMenuItem(isEnabled: boolean): IMenuItem {
    const enableLabel = t(
      __DARWIN__ ? 'Enable Commit Spellcheck' : 'Enable commit spellcheck'
    )
    const disableLabel = t(
      __DARWIN__ ? 'Disable Commit Spellcheck' : 'Disable commit spellcheck'
    )
    return {
  ...
  private renderCopilotButton() {
    ...
    const ariaLabel = isGeneratingCommitMessage
      ? t('Generating commit details…')
      : t('Generate commit message with Copilot') +
        (noChangesAvailable
          ? t('. Files must be selected to generate a commit message.')
          : '')
    ...
    return (
      <>
        ...
        >
          <AriaLiveContainer
            message={
              isGeneratingCommitMessage ? t('Generating commit details…') : ''
            }
          />
  ...
  private renderCommitOptionsButton() {
    const ariaLabel = t('Configure commit options')
  ...
  private onCommitOptionsButtonClick = (
    ...
    if (enableHooksEnvironment()) {
      items.push({
        ...
        label: t(__DARWIN__ ? 'Bypass Commit Hooks' : 'Bypass Commit hooks'),
        action: () => {
    ...
    if (this.props.showAllowEmptyCommitOption) {
      items.push({
        ...
        label: t(__DARWIN__ ? 'Allow Empty Commit' : 'Allow empty commit'),
        action: () => {
          this.props.onUpdateCommitOptions(this.props.repository, {
    ...
    if (commitToAmend !== null) {
      return (
        <CommitWarning icon={CommitWarningIcon.Information}>
          <Trans i18nKey='commit-message.amend-notice'>
    ...
    if (showNoWriteAccess) {
      ...
      return (
        <CommitWarning
          icon={canBypass ? CommitWarningIcon.Warning : CommitWarningIcon.Error}
        >
          <Trans i18nKey='commit-message.commit-signing-rules'>
            <RepoRulesetsForBranchLink
              repository={repository.gitHubRepository}
              branch={branch}
            >
      ...
      return (
        <CommitWarning
          icon={canBypass ? CommitWarningIcon.Warning : CommitWarningIcon.Error}
        >
          <Trans i18nKey='commit-message.basic-rules'>
            <RepoRulesetsForBranchLink
              repository={repository.gitHubRepository}
              branch={branch}
            >
            ...
            {canBypass ? t('would') : t('will')} prevent pushing
            {canBypass && t(', but you can bypass them. Proceed with caution!')}
            {!canBypass && (
              <>
                {t('. Want to')}{' '}
                <LinkButton onClick={this.onSwitchBranch}>
    ...
    return (
      ...
      >
        ...
        <RepoRulesMetadataFailureList
          ...
          leadingText={t('This commit message')}
        />
      </Popover>
    )
  }
  ...
  private getCommittingButtonText() {
    ...
    const action = t('{{verb}} {{files}}to ', {
      verb,
      files: this.getFilesToBeCommittedButtonText(),
    })
  ...
  private getFilesToBeCommittedButtonText() {
    ...
    return t(
      filesToBeCommittedCount > 1 ? '{{count}} files ' : '{{count}} file ',
      { count: filesToBeCommittedCount }
    )
  }
  ...
  private getCommittingButtonTitle() {
    ...
    return t('{{verb}} to {{branch}}', { verb, branch })
  }
  ...
  private getButtonTitle(): string {
    ...
    const isAmending = commitToAmend !== null
    return isAmending
      ? t('{{verb}} last commit', { verb: this.getButtonVerb() })
  ...
  private getButtonTooltip(buttonEnabled: boolean) {
    ...
    if (isSummaryBlank) {
      return t('A commit summary is required to commit')
    } else if (
      !this.props.anyFilesSelected &&
      this.props.anyFilesAvailable &&
      !this.props.allowEmptyCommit
    ) {
      return t('Select one or more files to commit')
    } else if (this.props.isCommitting) {
      return t('Committing changes…')
    }
    ...
    return (
      <ToggledtippedContent
        ...
        tooltip={
          <>
            <div className="title">
              {t('Great commit summaries contain fewer than 50 characters')}
            </div>
            <div className="description">
              {t('Place extra information in the description field.')}
            </div>
          </>
        }
        ariaLiveMessage={t(
        ...
        ariaLabel={t('Open Summary Length Info')}
      >
  ...
  private renderRepoRuleCommitMessageFailureHint(): JSX.Element | null {
    ...
    const ariaLabel = canBypass
      ? t(
          'Warning: Commit message fails repository rules, but you can bypass them. View details.'
        )
      : t('Error: Commit message fails repository rules. View details.')
  ...
  private renderCommitProgress() {
    ...
    const text =
      hookName === 'pre-auto-gc' && status === 'finished'
        ? t('Optimizing repository…')
        : status === 'started'
        ? t('{{hookName}} hook running…', { hookName })
        : status === 'finished'
        ? t('{{hookName}} hook finished', { hookName })
        : status === 'failed'
        ? t('{{hookName}} hook failed', { hookName })
    ...
    return (
      <div className={cn}>
        ...
        {onShowCommitProgress && (
          <Button
            tooltip={t('Show commit progress')}
      ...
      >
        <div className={summaryClassName} ref={this.summaryGroupRef}>
          ...
          <AutocompletingInput
            required={true}
            label={
              this.props.showInputLabels === true ? t('Summary') : undefined
            }
            screenReaderLabel={t('Commit summary')}
