...
const RepositorySettings = React.lazy(() =>
  import('./repository-settings').then(m => ({ default: m.RepositorySettings }))
)
import { AppError } from './app-error'
import { MissingRepository } from './missing-repository'
import { AddExistingRepository, CreateRepository } from './add-repository'
const CloneRepository = React.lazy(() =>
  import('./clone-repository').then(m => ({ default: m.CloneRepository }))
)
...
const Acknowledgements = React.lazy(() =>
  import('./acknowledgements').then(m => ({ default: m.Acknowledgements }))
)
...
const ReleaseNotes = React.lazy(() =>
  import('./release-notes').then(m => ({ default: m.ReleaseNotes }))
)
...
const OpenPullRequestDialog = React.lazy(() =>
  import('./open-pull-request/open-pull-request-dialog').then(m => ({
    default: m.OpenPullRequestDialog,
  }))
)
...
export class App extends React.Component<IAppProps, IAppState> {
  ...
  public constructor(props: IAppProps) {
    ...
    props.dispatcher
      ...
      .catch(e => {
        // Never leave the window permanently blank: if initial state fails to
        // load, surface the error and still paint the shell.
        log.error('Failed to load initial application state', e)
        this.loading = false
        this.forceUpdate()
      })
  ...
  private async performDeferredLaunchActions() {
    // Check git availability after startup completes (non-blocking).
    isGitOnPath().then(found => {
      if (!found) {
        this.props.dispatcher.showPopup({ type: PopupType.InstallGit, path: '' })
      }
    })
      ...
      case PopupType.Preferences:
        ...
        return (
          <Preferences
            ...
            customEditors={this.state.customEditors}
            customShells={this.state.customShells}
            recentReposDisplayCount={this.state.recentReposDisplayCount}
      ...
      case PopupType.GenerateCommitMessageDisclaimer: {
        ...
        return (
          <CopilotDisclaimer
            ...
            onDismissed={onPopupDismissedFn}
          >
            {t('Review and edit the generated message carefully before use.')}
          </CopilotDisclaimer>
        )
      }
  ...
  private renderRepositoryList = (): JSX.Element => {
    ...
    return (
      <RepositoriesList
        ...
        repoLastAccessed={this.state.repoLastAccessed}
        recentReposDisplayCount={this.state.recentReposDisplayCount}
  ...
  private openInCustomEditor = (
    ...
    this.props.dispatcher.openInCustomEditor(repository.path, editor)
  }

  private openInCustomShell = (
    repository: Repository | CloningRepository,
    ...
    this.props.dispatcher.openInCustomShell(repository.path, shell)
  }
  ...
  private renderRepositoryToolbarButton() {
    ...
    if (repository) {
      ...
      icon = octicons.repo
      title = t(__DARWIN__ ? 'Select a Repository' : 'Select a repository')
    } else {
      icon = octicons.repo
      title = t(__DARWIN__ ? 'No Repositories' : 'No repositories')
    }
    ...
    return (
      <ToolbarDropdown
        icon={icon}
        title={title}
        description={t(__DARWIN__ ? 'Current Repository' : 'Current repository')}
        tooltip={tooltip}
  ...
  private onRepositoryToolbarButtonContextMenu = () => {
    ...
    const items = generateRepositoryListContextMenu({
      ...
      customEditors: this.state.customEditors,
      customShells: this.state.customShells,
      onOpenInCustomEditor: this.openInCustomEditor,
      onOpenInCustomShell: this.openInCustomShell,
    })
...
function NoRepositorySelected() {
  return <div className="panel blankslate">{t('No repository selected')}</div>
}
