...
export function getCombinedStatusSummary(
  ...
  const conclusions = Object.values(groupBy(statusHolders, 'conclusion')).map(
    g =>
      t('{{count}} {{adjective}}', {
        count: g.length,
        adjective: getCheckRunConclusionAdjective(g[0].conclusion).toLocaleLowerCase(),
      })
  )

  const summary = toSentence(conclusions)
  if (description === undefined) return summary

  const plural = statusHolders.length !== 1
  const pluralDesc =
    description === 'check'
      ? plural ? t('checks') : t('check')
      : plural ? t('steps') : t('step')

  return t('{{summary}} {{pluralDesc}}', { summary, pluralDesc })
}
...
> {
  ...
  private renderCheckRunLoadings(): JSX.Element {
    return (
      <div className="loading-check-runs">
        ...
        <div className="title">{t('Stand By')}</div>
        <div className="call-to-action">{t('Check runs incoming!')}</div>
      </div>
    )
  }
  ...
  private renderCompletenessIndicator(
    ...
    const ariaLabel = t(
      'Completeness indicator. {{completed}} completed, {{inProgress}} in progress, {{queued}} queued.',
      {
  ...
  private getTitle(
    ...
    switch (true) {
      case loading:
        return <>{t('Checks Summary')}</>
      case somePendingNoFailures:
        return (
          <span className="pending">
            {t("Some checks haven't completed yet")}
          </span>
        )
      case allFailure:
        return <span className="failure">{t('All checks have failed')}</span>
      case allSuccess:
        return <>{t('All checks have passed')}</>
    }

    return (
      <span className="failure">{t('Some checks were not successful')}</span>
    )
  }
