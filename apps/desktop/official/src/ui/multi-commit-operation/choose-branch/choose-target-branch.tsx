...
> {
  ...
  private renderOkButtonText() {
    ...
    if (isCreateBranchState) {
      return t(
        __DARWIN__ ? 'Cherry-pick to New Branch' : 'Cherry-pick to new branch'
      )
    }
    ...
    if (selectedBranch !== null) {
      ...
      return (
        <>
          <Trans i18nKey='choose-target-branch.ok-button-to-branch'>
            {{ okButtonText }} to <strong>{branchName}</strong>…
          </Trans>
        </>
      )
    }
  ...
  public render() {
    const tooltip = this.selectedBranchIsCurrentBranch()
      ? t('You are not able to cherry-pick from and to the same branch')
      : undefined
      ...
      <Dialog
        ...
        title={
          <strong>
            {t(
              this.props.commitCount > 1
                ? 'Cherry-pick {{count}} commits to a branch'
                : 'Cherry-pick {{count}} commit to a branch',
              { count: this.props.commitCount }
            )}
          </strong>
        }
