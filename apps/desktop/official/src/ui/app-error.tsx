...
export class AppError extends React.Component<IAppErrorProps, IAppErrorState> {
    ...
    if (
      ...
      return (
        <>
          ...
          {files.length > 0 && (
            <>
              <p>{t('Files that exceed the limit')}</p>
              <ul>
                {files.map(file => (
                  <li key={file}>{file}</li>
                ))}
          ...
          <p>
            <Trans i18nKey='app-error.lfs-more-info'>
              See{' '}
  ...
  private getTitle(error: Error) {
    ...
    switch (getDugiteError(error)) {
      case DugiteError.PushWithFileSizeExceedingLimit:
        return t('File size limit exceeded')
    }

    switch (getRetryActionType(error)) {
      case RetryActionType.Clone:
        return t('Clone failed')
      case RetryActionType.Push:
        return t('Failed to push')
    }
    ...
    if (isErrorWithMetaData(error)) {
      ...
      switch (gitContext?.kind) {
        case 'create-repository':
          return t('Failed creating repository')
        case 'commit':
          return t('Commit failed')
      }
    }
  ...
  private renderContentAfterErrorMessage(error: Error) {
    ...
    if (retryAction && retryAction.type === RetryActionType.Clone) {
      return (
        <p>
          <Trans i18nKey='app-error.retry-cloning'>
  ...
  private renderRetryCloneFooter() {
    return (
      <DialogFooter>
        <OkCancelButtonGroup
          okButtonText={t(__DARWIN__ ? 'Retry Clone' : 'Retry clone')}
    ...
    return (
      <DialogFooter>
        <OkCancelButtonGroup
          ...
          cancelButtonText={t(__DARWIN__ ? 'Open Preferences' : 'Open options')}
