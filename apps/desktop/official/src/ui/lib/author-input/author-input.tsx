...
> {
  ...
  private removeAuthor(index: number, direction: 'back' | 'forward' | 'none') {
    ...
    let actionDescription = t('Removed {{username}}', {
      username: authorToRemove.username,
    })
    if (isKnownAuthor(authorToRemove)) {
      actionDescription += t(' ({{name}})', { name: authorToRemove.name })
    }
  ...
  private async attemptUnknownAuthorSearch(author: UnknownAuthor) {
    ...
    if (hit === null || hit.kind !== 'known-user') {
      ...
      this.setState({
        lastActionDescription: t('Error: user {{username}} not found', {
