...
export class AuthorHandle extends React.Component<IAuthorHandleProps> {
  ...
  private getAriaLabel() {
    ...
    if (isKnownAuthor(author)) {
      return t('{{name}} press backspace or delete to remove', {
        name: getFullTextForAuthor(author),
      })
    }

    const isError = author.state === 'error'
    const stateAriaLabel = isError ? t('user not found') : t('searching')
    return t(
      '{{username}}, {{state}}, press backspace or delete to remove',
      {
  ...
  private getTitle() {
    ...
    return author.state === 'error'
      ? t('Could not find user with username {{username}}', {
          username: author.username,
        })
      : t('Searching for @{{username}}', { username: author.username })
  }
