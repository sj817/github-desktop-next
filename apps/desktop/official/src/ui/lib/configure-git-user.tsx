  ...
  private renderExampleCommit() {
    ...
    return (
      <div id="commit-list" className="commit-list-example">
        <div className="header">{t('Example commit')}</div>
  ...
  private renderAuthorOptions() {
    ...
    return (
      <div>
        <RadioButton
          label={t(
            'Use my GitHub{{accountTypeSuffix}} account name and email address',
            { accountTypeSuffix }
          )}
  ...
  private renderConfigForm() {
    return (
      <Form className="sign-in-form" onSubmit={this.save}>
        ...
        <Row>
          <Button type="submit">{this.props.saveLabel || t('Save')}</Button>
          {this.props.children}
        </Row>
      </Form>
    )
