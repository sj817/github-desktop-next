...
const defaultConflictsResolvedMessage = t('No conflicts remaining')
...
const renderManualConflictedFile: React.FunctionComponent<{
  ...
  if ([entry.us, entry.them].includes(GitStatusEntry.Deleted)) {
    let targetBranch = t('target branch')
    ...
    conflictTypeString = t('File does not exist on {{targetBranch}}.', {
      targetBranch,
    })
  }
...
const renderConflictedFileWithConflictMarkers: React.FunctionComponent<{
  ...
  const message =
    humanReadableConflicts === 1
      ? t('{{humanReadableConflicts}} conflict', { humanReadableConflicts })
      : t('{{humanReadableConflicts}} conflicts', { humanReadableConflicts })
    ...
    <>
      ...
      <div className="action-buttons">
        ...
        <Button
          ...
          ariaLabel={t('File resolution options')}
          ariaHaspopup="menu"
...
function editorButtonString(editorName: string | null): string {
  const defaultEditorString = t('editor')
  return t('Open in {{editorName}}', {
    editorName: editorName || defaultEditorString,
  })
}
...
function editorButtonTooltip(editorName: string | null): string | undefined {
  ...
  if (__DARWIN__) {
    return t('No editor configured in Preferences > Advanced')
  } else {
    return t('No editor configured in Options > Advanced')
  }
}

const manualConflictString = t('Manual conflict')
