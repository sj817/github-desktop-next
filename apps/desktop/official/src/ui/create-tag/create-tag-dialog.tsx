...
> {
  ...
  public render() {
    ...
    return (
      <Dialog
        id="create-tag"
        title={t(__DARWIN__ ? 'Create a Tag' : 'Create a tag')}
      ...
      >
        ...
        <DialogFooter>
          <OkCancelButtonGroup
            okButtonText={t(__DARWIN__ ? 'Create Tag' : 'Create tag')}
  ...
  private renderPreviousTags() {
    ...
    const title = t(__DARWIN__ ? 'Previous Tags' : 'Previous tags')
    ...
    return (
      <>
        ...
        {lastThreeTags.length === 0 ? (
          <p>{t("No matches found for '{{tagName}}'", { tagName })}</p>
        ) : (
  ...
  private getCurrentError(): JSX.Element | null {
    if (this.state.tagName.length > MaxTagNameLength) {
      return (
        <>
          {t('The tag name cannot be longer than {{count}} characters', {
    ...
    if (alreadyExists) {
      return (
        <Trans i18nKey='create-tag.tag-already-exists'>
