    ...
    return (
      <div className="tutorial-panel-component panel">
        <div className="titleArea">
          <h3>{t('Get started')}</h3>
          <img src={TutorialPanelImage} alt={t('Partially checked check list')} />
        </div>
        <ol>
          <TutorialStepInstructions
            summaryText={t('Install a text editor')}
            ...
            {!this.isStepComplete(TutorialStep.PickEditor) ? (
              <>
                <p className="description">
                  <Trans i18nKey='tutorial-panel.recommend-editor'>
                    It doesn’t look like you have a text editor installed. We can
                    recommend{' '}
                    <LinkButton
                      uri={suggestedExternalEditor.url}
                      title={t('Open the {{editorName}} website', {
                        editorName: suggestedExternalEditor.name,
                      })}
                    >
              ...
              <p className="description">
                <Trans i18nKey='tutorial-panel.default-editor'>
                  ...
                  <LinkButton onClick={this.onPreferencesClick}>
                    {__DARWIN__ ? t('Settings') : t('options')}
                  </LinkButton>
                </Trans>
              </p>
            )}
          ...
          >
            <p className="description">
              {t(
                'A branch allows you to work on different versions of a repository at one time. Create a branch by going into the branch menu in the top bar and clicking "{{newBranch}}".',
                { newBranch: t(__DARWIN__ ? 'New Branch' : 'New branch') }
              )}
            </p>
            <div className="action">
              <KeyboardShortcut
          ...
          >
            <p className="description">
              <Trans i18nKey='tutorial-panel.edit-file'>
            ...
            {this.props.resolvedExternalEditor && (
              <div className="action">
                <Button onClick={this.openTutorialFileInEditor}>
                  {t(__DARWIN__ ? 'Open Editor' : 'Open editor')}
                </Button>
                <KeyboardShortcut
                  darwinKeys={['⌘', '⇧', 'A']}
                  keys={['Ctrl', 'Shift', 'A']}
          ...
          <TutorialStepInstructions
            summaryText={t('Publish to GitHub')}
            isComplete={this.isStepComplete}
            isNextStepTodo={this.isStepNextTodo}
          ...
          <TutorialStepInstructions
            summaryText={t('Open a pull request')}
            isComplete={this.isStepComplete}
            isNextStepTodo={this.isStepNextTodo}
          ...
          >
            ...
            <div className="action">
              <Button onClick={this.openPullRequest} role="link">
                {t(__DARWIN__ ? 'Open Pull Request' : 'Open pull request')}
        ...
        <div className="footer">
          <Button onClick={this.props.onExitTutorial}>
            {t(__DARWIN__ ? 'Exit Tutorial' : 'Exit tutorial')}
          </Button>
        </div>
      </div>
    )
...
const SkipLinkButton: React.FunctionComponent<{
  onClick: () => void
}> = props => <LinkButton onClick={props.onClick}>{t('Skip')}</LinkButton>
