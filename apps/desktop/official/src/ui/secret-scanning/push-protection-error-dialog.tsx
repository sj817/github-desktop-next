...
> {
      ...
      >
        <DialogContent>
          <div id="push-protection-error-dialog-description">
            <p>
              <Trans i18nKey='push-protection.secret-scanning-found'>
            ...
            <p>
              <Trans i18nKey='push-protection.allowing-secrets-risks'>
            ...
            {t('Exposing this secret can allow someone to:')}
            <ul>
              <li>{t('Verify the identity of the secret(s)')}</li>
              <li>{t('Know which resources the secret(s) can access')}</li>
              <li>{t("Act on behalf of the secret's owner")}</li>
              <li>
                {t('Push the secret(s) to this repository without being blocked')}
              </li>
            </ul>
            {this.renderSecrets()}
          </div>
  ...
  private renderBypassButton = (secret: ISecretScanResult) => {
    if (secret.requiresApproval) {
      return (
        <LinkButton
          ariaLabel={t('Bypass {{description}}', {
    ...
    if (this.state.secretsBypassed.get(secret.id)) {
      return (
        <span className="bypass-success">
          <Trans i18nKey='push-protection.bypassed'>
            Bypassed{' '}
  ...
  private renderSecrets = () => {
    ...
    return (
      <ul aria-label={t('Secrets')} className="secret-list">
        {listItems}
      </ul>
    )
  }
