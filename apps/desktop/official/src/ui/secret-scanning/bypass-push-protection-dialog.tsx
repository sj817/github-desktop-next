...
> {
  ...
  public render() {
    const items = [
      ...
      {
        title: t("It's a false positive"),
        description: t('The detected string is not a secret'),
        key: BypassReason.FalsePositive,
      },
      {
        title: t("I'll fix it later"),
        description: t(
    ...
    return (
      <Dialog
        title={t(
          __DARWIN__ ? 'Bypass Push Detection' : 'Bypass push detection'
        )}
      ...
      >
        <DialogContent>
          <VerticalSegmentedControl
            label={t('Why are you bypassing this {{description}}?', {
        ...
        <DialogFooter>
          <OkCancelButtonGroup
            okButtonText={t('Allow me to expose this secret')}
            destructive={true}
          />
        </DialogFooter>
      </Dialog>
