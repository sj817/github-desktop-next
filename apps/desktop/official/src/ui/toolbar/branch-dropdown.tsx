...
export class BranchDropdown extends React.Component<IBranchDropdownProps> {
  ...
  public render() {
    ...
    let title: string
    let description = t(__DARWIN__ ? 'Current Branch' : 'Current branch')
    let canOpen = true
    let disabled = false
    let tooltip: string
    ...
    if (tip.kind === TipState.Unknown) {
      ...
      title = tip.ref
      tooltip = t('Current branch is {{ref}}', { ref: tip.ref })
      canOpen = branchesState.allBranches.some(
        b => !b.isDesktopForkRemoteBranch
      )
    } else if (tip.kind === TipState.Detached) {
      title = t('On {{sha}}', { sha: tip.currentSha.substring(0, 7) })
      tooltip = t('Currently on a detached HEAD')
      icon = octicons.gitCommit
      description = t('Detached HEAD')
    ...
    if (checkoutProgress) {
      ...
      tooltip = t('Checking out {{branch}}', { branch: checkoutProgress.target })
      ...
      description = t('Rebasing branch')
      icon = octicons.gitBranch
      canOpen = false
      disabled = true
      tooltip = t('Rebasing {{branch}}', { branch: conflictState.targetBranch })
    }
    ...
    return (
      <>
        <Resizable
          ...
          description={t('Current branch dropdown button')}
        >
          <ToolbarDropdown
            className="branch-button"
            icon={icon}
