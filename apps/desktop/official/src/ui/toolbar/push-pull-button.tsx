...
function renderLastFetched(lastFetched: Date | null): JSX.Element | string {
  if (lastFetched) {
    return (
      <span>
        <Trans i18nKey='push-pull-button.last-fetched'>
          Last fetched <RelativeTime date={lastFetched} />
        </Trans>
      </span>
    )
  } else {
    return t('Never fetched')
  }
}
...
> {
  ...
  > {
    return {
      ...
      ariaLabel: t('Push, pull, fetch options'),
    ...
    return (
      <>
        <Resizable
          ...
          description={t('Push pull button')}
        >
          {this.renderButton()}
          <span id="push-pull-button-state">
            <AriaLiveContainer message={this.state.screenReaderStateMessage} />
  ...
  private progressButton(progress: Progress, networkActionInProgress: boolean) {
    return (
      <ToolbarButton
        ...
        description={progress.description || t('Hang on…')}
  ...
  private publishRepositoryButton(onClick: () => void) {
    return (
      <ToolbarButton
        ...
        description={t('Publish this repository to GitHub')}
        className="push-pull-button"
  ...
  private detachedHeadButton(rebaseInProgress: boolean) {
    const description = rebaseInProgress
      ...
      : t('Cannot publish detached HEAD')

    return (
      <ToolbarButton
        {...this.defaultButtonProps()}
        title={t('Publish branch')}
  ...
  private publishBranchButton(
    ...
    const description = isGitHub
      ? t('Publish this branch to GitHub')
      : t('Publish this branch to the remote')
  ...
  private fetchButton(
    ...
    const title = t('Fetch {{remoteName}}', { remoteName })
    return (
      <ToolbarButton
        {...this.defaultButtonProps()}
        title={title}
  ...
  private pullButton(
    ...
    const title = pullWithRebase
      ? t('Pull {{remoteName}} with rebase', { remoteName })
      : t('Pull {{remoteName}}', { remoteName })
  ...
  private pushButton(
    ...
    return (
      <ToolbarDropdown
        {...this.defaultDropdownProps()}
        title={t('Push {{remoteName}}', { remoteName })}
        description={renderLastFetched(lastFetched)}
  ...
  private forcePushButton(
    ...
    return (
      <ToolbarDropdown
        {...this.defaultDropdownProps()}
        title={t('Force push {{remoteName}}', { remoteName })}
        description={renderLastFetched(lastFetched)}
        icon={forcePushIcon}
        onClick={onClick}
        dropdownContentRenderer={this.getDropdownContentRenderer([
