...
> {
  ...
  public render() {
    ...
    return (
      <Dialog
        ...
        title={t(__DARWIN__ ? 'Clone a Repository' : 'Clone a repository')}
        onSubmit={this.clone}
      ...
      >
        ...
        >
          <span id="dotcom-tab">{t('GitHub.com')}</span>
          <span id="enterprise-tab">{t('GitHub Enterprise')}</span>
  ...
  private renderSignIn(tab: CloneRepositoryTab) {
    const signInTitle = t(__DARWIN__ ? 'Sign In' : 'Sign in')
    switch (tab) {
      case CloneRepositoryTab.DotCom:
        return (
  ...
  private clone = async () => {
    ...
    if (path == null) {
      const error = new Error(t('Directory could not be created at this path.'))
    ...
    if (!cloneInfo) {
      const error = new Error(
        t("We couldn't find that repository. Check that you are logged in, the network is accessible, and the URL or repository alias are spelled correctly.")
      )
      this.setState({ loading: false })
      this.setSelectedTabState({ error })
      return
