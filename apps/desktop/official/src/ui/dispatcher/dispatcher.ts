...
export class Dispatcher {
  ...
  /** Switch the active account (auth, API, and commit identity). */
  ...
  /** Set the list of custom editors */
  public setCustomEditors(editors: ReadonlyArray<ICustomIntegration>) {
    this.appStore._setCustomEditors(editors)
  }

  /** Set the list of custom shells */
  public setCustomShells(shells: ReadonlyArray<ICustomIntegration>) {
    this.appStore._setCustomShells(shells)
  }

  public setRecentReposDisplayCount(count: number) {
    this.appStore._setRecentReposDisplayCount(count)
  }

  /** Open a repository in a specific custom editor */
  public async openInCustomEditor(
    path: string,
    editor: ICustomIntegration
  ) {
    await this.appStore._openInCustomEditor(path, editor)
  }

  /** Open a repository in a specific custom shell */
  public async openInCustomShell(
    path: string,
    shell: ICustomIntegration
  ) {
    await this.appStore._openInCustomShell(path, shell)
  }
