...
export class OpenPullRequestDialog extends React.Component<IOpenPullRequestDialogProps> {
  ...
  private renderNoChanges() {
    ...
    const message = hasMergeBase ? (
      <Trans i18nKey='open-pull-request-dialog.up-to-date'>
        <Ref>{baseBranch.name}</Ref> is up to date with all commits from{' '}
        <Ref>{currentBranch.name}</Ref>.
      </Trans>
    ) : (
      <Trans i18nKey='open-pull-request-dialog.different-histories'>
  ...
  private renderNoDefaultBranch() {
    ...
    return (
      <div className="open-pull-request-message">
        <div>
          <Octicon symbol={octicons.gitPullRequest} />
          <h3>{t('Could not find a default branch to compare against.')}</h3>
          {t('Select a base branch above.')}
        </div>
      </div>
    )
  }
  ...
  private renderFooter() {
    ...
    const buttonTitle = currentBranchHasPullRequest
      ? isEnterprise
        ? t('View pull request on GitHub Enterprise.')
        : t('View pull request on GitHub.')
      : isEnterprise
      ? t('Create pull request on GitHub Enterprise.')
      : t('Create pull request on GitHub.')
    ...
    const okButton = (
      <>
        ...
        {currentBranchHasPullRequest
          ? t(__DARWIN__ ? 'View Pull Request' : 'View pull request')
          : t(__DARWIN__ ? 'Create Pull Request' : 'Create pull request')}
      </>
    )
