  ...
  public render() {
    return (
      <Dialog
        id="install-git"
        type="warning"
        title={t(__DARWIN__ ? 'Unable to Locate Git' : 'Unable to locate Git')}
        onSubmit={this.onSubmit}
        onDismissed={this.props.onDismissed}
      >
        <DialogContent>
          <p>
            <Trans i18nKey='install-git.unable-to-locate'>
              We were unable to locate Git on your system. This means you won't
              be able to execute any Git commands in the{' '}
              {{
                location: __DARWIN__
                  ? t('Terminal window')
                  : t('command prompt'),
        ...
        <DialogFooter>
          <OkCancelButtonGroup
            okButtonText={t(
              __DARWIN__ ? 'Open Without Git' : 'Open without Git'
            )}
            cancelButtonText={t('Install Git')}
