...
> {
  ...
  private getOkButtonLabel() {
    if (this.props.discardingAllChanges) {
      return t(__DARWIN__ ? 'Discard All Changes' : 'Discard all changes')
    }
    return t(__DARWIN__ ? 'Discard Changes' : 'Discard changes')
  }
  ...
  private getDialogTitle() {
    ...
    return t(__DARWIN__ ? 'Confirm Discard Changes' : 'Confirm discard changes')
  }
      ...
      >
        <DialogContent>
          ...
          <p id="discard-changes-confirmation-message">
            <Trans i18nKey='discard-changes.restore-from-trash'>
    ...
    if (this.props.files.length > MaxFilesToList) {
      return (
        <p id="discard-changes-confirmation-file-list">
          {t('Are you sure you want to discard all {{count}} changed files?', {
      ...
      return (
        <div id="discard-changes-confirmation-file-list">
          <p>{t('Are you sure you want to discard all changes to:')}</p>
