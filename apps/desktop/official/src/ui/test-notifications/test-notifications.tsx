...
> {
  ...
  private getTypeFriendlyName(type?: TestNotificationType): string {
    const titleMap = new Map<TestNotificationType, string>([
      [TestNotificationType.PullRequestReview, t('Pull Request Review')],
      [TestNotificationType.PullRequestComment, t('Pull Request Comment')],
      [TestNotificationType.ChecksFailed, t('Pull Request Checks Failed')],
    ])
  ...
  private renderCurrentStep() {
    if (this.state.selectedFlow === null) {
      return (
        <div>
          <p>{t('Select the type of notification to display:')}</p>
  ...
  private renderSelectPullRequest() {
    ...
    if (pullRequests.length === 0) {
      return <p>{t('No pull requests found')}</p>
    }
  ...
  private renderSelectPullRequestComment() {
    ...
    if (comments.length === 0) {
      return <p>{t('No comments found')}</p>
    }
