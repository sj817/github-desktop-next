...
> {
    ...
    (insertionIndexPath: RowIndexPath | null) => {
      ...
      if (insertionIndexPath !== null) {
        ...
        const insertionPoint =
          row < this.props.commitSHAs.length
            ? t('before commit {{n}}', { n: row + 1 })
            : t('after commit {{n}}', { n: row })

        this.setState({
          reorderingMessage:
            count === 1
              ? t(
                  'Press Enter to insert the selected commit {{insertionPoint}} or Escape to cancel.',
                  { insertionPoint }
                )
              : t(
                  'Press Enter to insert the selected commits {{insertionPoint}} or Escape to cancel.',
                  { insertionPoint }
                ),
        })
        return
      ...
      this.setState({
        reorderingMessage:
          count === 1
            ? t(
                'Use the Up and Down arrow keys to choose a new location for the selected commit, then press Enter to confirm or Escape to cancel.'
              )
            : t(
                'Use the Up and Down arrow keys to choose a new location for the selected commits, then press Enter to confirm or Escape to cancel.'
              ),
      })
    },
    500
  ...
  private getUnpushedIndicatorTitle(
    ...
    if (isLocalCommit) {
      return t('This commit has not been pushed to the remote repository')
    }

    if (numUnpushedTags > 0) {
      return numUnpushedTags === 1
        ? t('This commit has {{n}} tag to push', { n: numUnpushedTags })
        : t('This commit has {{n}} tags to push', { n: numUnpushedTags })
    }
    ...
    if (commitSHAs.length === 0) {
      return (
        <div className="panel blankslate">
          {emptyListMessage ?? t('No commits to list')}
        </div>
      )
    }
  ...
  private renderReorderCommitsHint = () => {
    ...
    const reorderCommitsHintTitle = t(
      __DARWIN__ ? 'Reorder Commits' : 'Reorder commits'
    )
    ...
    return (
      ...
      >
        ...
        <p>
          <Trans i18nKey='commit-list.reorder-hint-choose-location'>
        ...
        <p>
          <Trans i18nKey='commit-list.reorder-hint-confirm'>
            Press <KeyboardShortcut darwinKeys={['⏎']} keys={['⏎']} /> to
            confirm.
          </Trans>
        </p>
      </Popover>
  ...
  private getContextMenuForSingleCommit(
    ...
    let viewOnGitHubLabel = t('View on GitHub')
    ...
    if (
      ...
      viewOnGitHubLabel = t('View on GitHub Enterprise')
    }
    ...
    if (canBeAmended) {
      items.push({
        label: t(__DARWIN__ ? 'Amend Commit…' : 'Amend commit…'),
    ...
    if (canBeUndone) {
      items.push({
        label: t(__DARWIN__ ? 'Undo Commit…' : 'Undo commit…'),
        action: () => {
    ...
    items.push({
      label: t(__DARWIN__ ? 'Reset to Commit…' : 'Reset to commit…'),
      action: () => {
    ...
    items.push({
      label: t(__DARWIN__ ? 'Checkout Commit' : 'Checkout commit'),
      action: () => {
    ...
    items.push({
      label: t(__DARWIN__ ? 'Reorder Commit' : 'Reorder commit'),
      action: () => {
    ...
    const darwinTagsLabel =
      commit.tags.length > 1 ? t('Copy Tags') : t('Copy Tag')
    const windowTagsLabel =
      commit.tags.length > 1 ? t('Copy tags') : t('Copy tag')
    items.push(
      {
        label: t(__DARWIN__ ? 'Cherry-pick Commit…' : 'Cherry-pick commit…'),
  ...
  private getDeleteTagsMenuItem(commit: Commit): IMenuItem | null {
    ...
    if (commit.tags.length === 1) {
      ...
      return {
        label: t('Delete tag {{tagName}}', { tagName }),
  ...
  private getContextMenuMultipleCommits(commit: Commit): IMenuItem[] {
    ...
    return [
      ...
      {
        label: t(
          __DARWIN__ ? 'Squash {{count}} Commits…' : 'Squash {{count}} commits…',
          { count }
        ),
