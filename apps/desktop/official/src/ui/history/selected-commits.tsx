...
> {
  ...
  private renderFileList() {
    const files = this.props.changesetData.files
    if (files.length === 0) {
      return <div className="fill-window">{t('No files in commit')}</div>
    }
      ...
      <div id="history" className={className}>
        ...
        <div className="commit-details">
          <Resizable
            ...
            description={t('Selected commit file list')}
          >
            {this.renderFileList()}
          </Resizable>
          {this.renderDiff()}
        ...
        <div className="panel blankslate">
          ...
          <div>
            ...
            <ul>
              ...
              <li>
                {t('Drag the commits to the branch menu to cherry-pick them.')}
              </li>
              <li>{t('Drag the commits to squash or reorder them.')}</li>
              <li>{t('Right click on multiple commits to see options.')}</li>
            </ul>
          </div>
        </div>
        {this.renderDragOverlay()}
  ...
  private onContextMenu = async (
    ...
    const openInExternalEditor = externalEditorLabel
      ? t('Open in {{editor}}', { editor: externalEditorLabel })
      : DefaultEditorLabel()
