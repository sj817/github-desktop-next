...
> {
    ...
    return (
      <div className="merge-cta">
        ...
        <DropdownSelectButton
          ...
          dropdownAriaLabel={t('Merge options')}
          disabled={disabled}
  ...
  private renderLoadingMessage() {
    return (
      <div className="merge-message merge-message-loading">
        {t('Checking for ability to {{operation}} automatically…', {
  ...
  private renderCleanMessage(currentBranch: Branch, branch: Branch) {
    ...
    if (this.state.selectedOperation === MultiCommitOperationKind.Rebase) {
      return (
        <div className="merge-message">
          <Trans i18nKey='merge-cta.rebase-clean-message'>
    ...
    return (
      <div className="merge-message">
        <Trans i18nKey='merge-cta.merge-clean-message'>
          This will merge
          <strong>{` ${this.commitCount} ${pluralized}`}</strong>
          {` from `}
          <strong>{branch.name}</strong>
  ...
  private renderInvalidMessage() {
    if (this.state.selectedOperation === MultiCommitOperationKind.Rebase) {
      return (
        <div className="merge-message">
          {t('Unable to start rebase. Check you have chosen a valid branch.')}
        </div>
      )
    }
  ...
  private renderConflictedMergeMessage(
    ...
    return (
      <div className="merge-message">
        <Trans i18nKey='merge-cta.conflicted-merge-message'>
          There will be
