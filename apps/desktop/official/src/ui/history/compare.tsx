...
> {
  ...
  private onDropCommitInsertion = async (
    ...
    if (
      ...
      defaultErrorHandler(
        new Error(
          t('Unable to reorder. Reordering replays all commits up to the last one required for the reorder. A merge commit cannot exist among those commits.')
        ),
  ...
  private onSquash = async (
    ...
    if (
      ...
      defaultErrorHandler(
        new Error(
          t('Unable to squash. Squashing replays all commits up to the last one required for the squash. A merge commit cannot exist among those commits.')
        ),
        this.props.dispatcher
      )
      return
    ...
    this.props.dispatcher.showPopup({
      ...
      dialogTitle: t('Squash {{count}} Commits', {
        count: allCommitsInSquash.length,
      }),
      dialogButtonText: t('Squash {{count}} Commits', {
        count: allCommitsInSquash.length,
      }),
...
function getPlaceholderText(state: ICompareState) {
  ...
  if (!branches.some(b => !b.isDesktopForkRemoteBranch)) {
    return t(__DARWIN__ ? 'No Branches to Compare' : 'No branches to compare')
