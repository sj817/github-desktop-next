...
export class CrashApp extends React.Component<ICrashAppProps, ICrashAppState> {
  ...
  private renderTitle() {
    const message =
      this.state.type === 'launch'
        ? t('GitHub Desktop failed to launch')
        : t('GitHub Desktop encountered an error')
  ...
  private renderDescription() {
    if (this.state.type === 'launch') {
      return (
        <p>
          <Trans i18nKey='crash-app.launch-description'>
            GitHub Desktop encountered a catastrophic error that prevents it
            from launching. This has been reported to the team, but if you
            encounter this repeatedly please report this issue to the GitHub
      ...
      return (
        <p>
          <Trans i18nKey='crash-app.error-description'>
            GitHub Desktop has encountered an unrecoverable error and will need
  ...
  private renderQuitButton() {
    ...
    if (__DEV__) {
      quitText = t(__DARWIN__ ? 'Quit' : 'Exit')
    } else {
      quitText = t(__DARWIN__ ? 'Quit and Restart' : 'Exit and restart')
    }
