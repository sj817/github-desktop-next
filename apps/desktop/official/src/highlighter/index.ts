...
// Must be first: defines a `require` shim so the Node-only require.cache line in
// codemirror's runmode.node.js doesn't throw "require is not defined" in this
// ESM worker (which would silently disable all syntax highlighting).
import './require-shim'
