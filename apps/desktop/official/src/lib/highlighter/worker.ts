...
// Vite bundles the highlighter worker (and its dynamically-imported CodeMirror
// modes) into a dedicated module-worker chunk. This replaces the webpack
// worker-loader artifact the official build referenced via
// encodePathAsUrl(__dirname, 'highlighter.js'), which does not exist under Vite.
// The URL is resolved at build time by Vite's `new Worker(new URL(...,
// import.meta.url), { type: 'module' })` detection.
const createHighlightWorker = () =>
  new Worker(new URL('../../highlighter/index.ts', import.meta.url), {
    type: 'module',
    name: 'highlighter',
  })
...
export function highlight(
  ...
  const worker = highlightWorkers.shift() || createHighlightWorker()
