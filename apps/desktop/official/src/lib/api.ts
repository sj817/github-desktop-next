...
export class API {
  ...
  public async createRepository(
    ...
    try {
      ...
      throw new Error(
        t('Unable to publish repository. Please check if you have an internet connection and try again.')
      )
    }
  }
    ...
    if (response.status === HttpStatusCode.TooManyRequests) {
      ...
      if (retryAfter) {
        throw new CopilotError(
          t('Rate limited, retry after {{retryAfter}} seconds.', { retryAfter }),
          response.status
        )
      } else {
        throw new CopilotError(
          t('Rate limited, try again in a few minutes.'),
          response.status
        )
      }
      ...
      throw new CopilotError(
        t('Unauthorized: error with authentication.'),
        response.status
      )
      ...
      if (body.includes('unauthorized: not licensed to use Copilot')) {
        throw new CopilotError(
          t('Unauthorized: not licensed to use Copilot.'),
          response.status
        )
      } else if (
        body.includes(
        ...
        throw new CopilotError(
          t('Unauthorized: not authorized to use this Copilot feature.'),
          response.status
        )
      } else if (
        ...
        throw new CopilotError(
          t('Integration does not have GitHub chat enabled.'),
          response.status
        )
      } else {
        throw new CopilotError(t('Unauthorized: unknown.'), response.status)
      }
    } else if (response.status === 466) {
      throw new CopilotError(
        t('Client issue: unsupported API version.'),
        response.status
      )
