...
export class DiffParser {
  ...
  private parseHunkHeader(line: string): DiffHunkHeader {
    ...
    if (!m) {
      throw new Error(t('Invalid hunk header format'))
    }
  ...
  private parseHunk(
    ...
    if (!headerLine) {
      throw new Error(t('Expected hunk header but reached end of diff'))
    }
    ...
    while ((c = this.parseLinePrefix(this.peek()))) {
      ...
      if (!line) {
        throw new Error(t('Expected unified diff line but reached end of diff'))
      }
    ...
    if (lines.length === 1) {
      throw new Error(t('Malformed diff, empty hunk'))
    }
