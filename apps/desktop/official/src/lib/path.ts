 ...
 *
 * In the Tauri build the renderer is served over http inside WebView2, so
 * assets must be addressed with root-relative URLs (e.g. /static/x.svg) rather
 * than file:// URLs. __dirname is defined as '/' (see vite.config.ts), so we
 * collapse the segments into a single absolute web path.
 *
