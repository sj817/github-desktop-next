...
async function findApplication(editor: WindowsExternalEditor) {
  for (const { key, subKey } of editor.registryKeys) {
    const keys = await enumerateValues(key, subKey)
    if (keys.length === 0) {
      continue
    }
...
const getJetBrainsToolboxEditors = memoizeOne(async () => {
  ...
  for (const parent of [uninstallSubKey, wow64UninstallSubKey]) {
    for (const key of await enumerateKeys(HKEY.HKEY_CURRENT_USER, parent)) {
      const m = re.exec(key)
      if (m) {
        ...
        const displayName = getKeyOrEmpty(
          await enumerateValues(HKEY.HKEY_CURRENT_USER, `${parent}\\${key}`),
          'DisplayName'
        )
        if (!displayName) {
