...
export async function launchShell(
  ...
  if (!exists) {
    const label = t(__DARWIN__ ? 'Settings' : 'Options')
    throw new ShellError(
      t(
        "Could not find executable for '{{shell}}' at path '{{path}}'. Please open {{label}} and select an available shell.",
        { shell: shell.shell, path: shell.path, label }
      )
    )
  }
...
export async function launchCustomShell(
  ...
  if (!exists) {
    ...
    throw new ShellError(
      t(
        "Could not find executable for custom shell at path '{{path}}'. Please open {{label}} and select an available shell.",
        { path: customShell.path, label }
      )
    )
  }
