...
async function findPowerShell(): Promise<string | null> {
  const powerShell = await enumerateValues(
...
async function findPowerShellCore(): Promise<string | null> {
  const powerShellCore = await enumerateValues(
    HKEY.HKEY_LOCAL_MACHINE,
...
async function findHyper(): Promise<string | null> {
  const hyper = await enumerateValues(
...
export async function findGitBash(): Promise<string | null> {
  const registryPath = await enumerateValues(
    HKEY.HKEY_LOCAL_MACHINE,
...
async function findCygwin(): Promise<string | null> {
  const registryPath64 = await enumerateValues(
    HKEY.HKEY_LOCAL_MACHINE,
    'SOFTWARE\\Cygwin\\setup'
  )
  const registryPath32 = await enumerateValues(
    HKEY.HKEY_LOCAL_MACHINE,
...
async function findWarp(): Promise<string | null> {
  const warpPresent = await enumerateValues(
    HKEY.HKEY_CURRENT_USER,
