...
export function parseError(error: Error): Error | null {
  if (squirrelMissingRegex.test(error.message)) {
    return new Error(
      t('The application is missing a dependency it needs to check and install updates. This is very, very bad.')
    )
  }
  if (squirrelDNSRegex.test(error.message)) {
    return new Error(
      t('GitHub Desktop was not able to contact the update server. Ensure you have internet connectivity and try again.')
    )
  }
  if (squirrelTimeoutRegex.test(error.message)) {
    return new Error(
      t('GitHub Desktop was not able to check for updates due to a timeout. Ensure you have internet connectivity and try again.')
    )
  }
