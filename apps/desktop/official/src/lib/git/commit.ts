...
export async function createCommit(
  ...
  options?: {
    ...
    allowEmpty?: boolean
    /**
     * Override the commit author/committer identity (used by the multi-account
     * fork so commits are attributed to the active account). When omitted, git's
     * configured user.name/user.email apply as usual.
     */
  ...
  const authorEnv = options?.author
    ? {
