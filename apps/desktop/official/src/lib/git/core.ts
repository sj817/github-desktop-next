...
export function getDescriptionForError(
  ...
  switch (error) {
    case DugiteError.BadConfigValue:
      ...
      if (errorInfo === null) {
        return t('Unsupported git configuration value.')
      }

      return t(
        "Unsupported value '{{value}}' for git config key '{{key}}'",
        { value: errorInfo.value, key: errorInfo.key }
      )
    case DugiteError.SSHKeyAuditUnverified:
      return t('The SSH key is unverified.')
    case DugiteError.RemoteDisconnection:
      return t('The remote disconnected. Check your Internet connection and try again.')
    case DugiteError.HostDown:
      return t('The host is down. Check your Internet connection and try again.')
    case DugiteError.RebaseConflicts:
      return t('We found some conflicts while trying to rebase. Please resolve the conflicts before continuing.')
    case DugiteError.MergeConflicts:
      return t('We found some conflicts while trying to merge. Please resolve the conflicts and commit the changes.')
    case DugiteError.HTTPSRepositoryNotFound:
    case DugiteError.SSHRepositoryNotFound:
      return t('The repository does not seem to exist anymore. You may not have access, or it may have been deleted or renamed.')
    case DugiteError.PushNotFastForward:
      return t('The repository has been updated since you last pulled. Try pulling before pushing.')
    case DugiteError.BranchDeletionFailed:
      return t('Could not delete the branch. It was probably already deleted.')
    case DugiteError.DefaultBranchDeletionFailed:
      return t("The branch is the repository's default branch and cannot be deleted.")
    case DugiteError.RevertConflicts:
      return t('To finish reverting, please merge and commit the changes.')
    case DugiteError.EmptyRebasePatch:
      return t("There aren't any changes left to apply.")
    case DugiteError.NoMatchingRemoteBranch:
      return t("There aren't any remote branches that match the current branch.")
    case DugiteError.NothingToCommit:
      return t('There are no changes to commit.')
    case DugiteError.NoSubmoduleMapping:
      return t('A submodule was removed from .gitmodules, but the folder still exists in the repository. Delete the folder, commit the change, then try again.')
    case DugiteError.SubmoduleRepositoryDoesNotExist:
      return t('A submodule points to a location which does not exist.')
    case DugiteError.InvalidSubmoduleSHA:
      return t('A submodule points to a commit which does not exist.')
    case DugiteError.LocalPermissionDenied:
      return t('Permission denied.')
    case DugiteError.InvalidMerge:
      return t('This is not something we can merge.')
    case DugiteError.InvalidRebase:
      return t('This is not something we can rebase.')
    case DugiteError.NonFastForwardMergeIntoEmptyHead:
      return t('The merge you attempted is not a fast-forward, so it cannot be performed on an empty branch.')
    case DugiteError.PatchDoesNotApply:
      return t('The requested changes conflict with one or more files in the repository.')
    case DugiteError.BranchAlreadyExists:
      return t('A branch with that name already exists.')
    case DugiteError.BadRevision:
      return t('Bad revision.')
    case DugiteError.NotAGitRepository:
      return t('This is not a git repository.')
    case DugiteError.ProtectedBranchForcePush:
      return t('This branch is protected from force-push operations.')
    case DugiteError.ProtectedBranchRequiresReview:
      return t('This branch is protected and any changes requires an approved review. Open a pull request with changes targeting this branch instead.')
    case DugiteError.PushWithFileSizeExceedingLimit:
      return t("The push operation includes a file which exceeds GitHub's file size restriction of 100MB. Please remove the file from history and try again.")
    case DugiteError.HexBranchNameRejected:
      return t('The branch name cannot be a 40-character string of hexadecimal characters, as this is the format that Git uses for representing objects.')
    case DugiteError.ForcePushRejected:
      return t('The force push has been rejected for the current branch.')
    case DugiteError.InvalidRefLength:
      return t('A ref cannot be longer than 255 characters.')
    case DugiteError.CannotMergeUnrelatedHistories:
      return t('Unable to merge unrelated histories in this repository.')
    case DugiteError.PushWithPrivateEmail:
      return t('Cannot push these commits as they contain an email address marked as private on GitHub. To push anyway, visit https://github.com/settings/emails, uncheck "Keep my email address private", then switch back to GitHub Desktop to push your commits. You can then enable the setting again.')
    case DugiteError.LFSAttributeDoesNotMatch:
      return t('Git LFS attribute found in global Git configuration does not match expected value.')
    case DugiteError.ProtectedBranchDeleteRejected:
      return t('This branch cannot be deleted from the remote repository because it is marked as protected.')
    case DugiteError.ProtectedBranchRequiredStatus:
      return t('The push was rejected by the remote server because a required status check has not been satisfied.')
    case DugiteError.BranchRenameFailed:
      return t('The branch could not be renamed.')
    case DugiteError.PathDoesNotExist:
      return t('The path does not exist on disk.')
    case DugiteError.InvalidObjectName:
      return t('The object was not found in the Git repository.')
    case DugiteError.OutsideRepository:
      return t('This path is not a valid path inside the repository.')
    case DugiteError.LockFileAlreadyExists:
      return t('A lock file already exists in the repository, which blocks this operation from completing.')
    case DugiteError.NoMergeToAbort:
      return t('There is no merge in progress, so there is nothing to abort.')
    case DugiteError.NoExistingRemoteBranch:
      return t('The remote branch does not exist.')
    case DugiteError.LocalChangesOverwritten:
      return t('Unable to switch branches as there are working directory changes which would be overwritten. Please commit or stash your changes.')
    case DugiteError.UnresolvedConflicts:
      return t('There are unresolved conflicts in the working directory.')
