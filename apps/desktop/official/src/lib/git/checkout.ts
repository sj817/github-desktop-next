...
export async function checkoutBranch(
  ...
  const title = t('Checking out branch {{name}}', { name: branch.name })
  const opts = await getCheckoutOpts(
    ...
    progressCallback
      ...
      : undefined,
    t(__DARWIN__ ? 'Switching to Branch' : 'Switching to branch')
  )
...
export async function checkoutCommit(
  ...
  const title = t(__DARWIN__ ? 'Checking out Commit' : 'Checking out commit')
