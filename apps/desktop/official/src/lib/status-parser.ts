...
function parseChangedEntry(field: string): IStatusEntry {
  ...
  if (!match) {
    ...
    throw new Error(t('Failed to parse status line for changed entry'))
  }
...
function parsedRenamedOrCopiedEntry(
  ...
  if (!match) {
    ...
    throw new Error(t('Failed to parse status line for renamed or copied entry'))
  }
...
function parseUnmergedEntry(field: string): IStatusEntry {
  ...
  if (!match) {
    ...
    throw new Error(t('Failed to parse status line for unmerged entry'))
  }
