...
const nl = __WIN32__ ? '\r\n' : '\n'
const getInitialReadmeContents = () =>
  `# ${t('Welcome to GitHub Desktop!')}${nl}${nl}` +
  `${t('This is your README. READMEs are where you can communicate what your project is and how to use it.')}${nl}${nl}` +
  `${t('Write your name on line 6, save it, and then head back to GitHub Desktop.')}${nl}`
    ...
    if (
      ...
      if (err.apiError.message === 'Repository creation failed.') {
        if (
          ...
          throw new Error(
            t('You already have a repository named "{{name}}" on your account at {{endpoint}}.\n\nPlease delete the repository and try again.', { name, endpoint: account.friendlyEndpoint })
          )
        }
      }
    }
...
async function pushRepo(
  ...
  const pushTitle = t('Pushing repository to {{endpoint}}', { endpoint: account.friendlyEndpoint })
...
export async function createTutorialRepository(
  ...
  progressCb(t('Creating repository on {{endpoint}}', { endpoint: account.friendlyEndpoint }), 0)

  if (await pathExists(path)) {
    throw new Error(
      t("The path '{{path}}' already exists. Please move it out of the way, or remove it, and then try again.", { path })
    )
  }
