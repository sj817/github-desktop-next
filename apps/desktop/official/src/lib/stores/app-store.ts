...
const RepoLastAccessedKey = 'repo-last-accessed'
const RecentReposDisplayCountKey = 'recent-repos-display-count'
const DefaultRecentReposDisplayCount = 10
const MaxRecentReposDisplayCount = 20
...
const customEditorsKey = 'custom-editors'
const customShellsKey = 'custom-shells'
...
export class AppStore extends TypedBaseStore<IAppState> {
  ...
  private repoLastAccessed: Map<number, number> = new Map()
  private recentReposDisplayCount: number = DefaultRecentReposDisplayCount
  ...
  private customEditors: ReadonlyArray<ICustomIntegration> = []
  private customShells: ReadonlyArray<ICustomIntegration> = []
  ...
  public constructor(
    ...
    // This fork does not force the welcome/sign-in flow. Start straight in the
    // app so it's usable without an account; sign-in stays available later via
    // Preferences > Accounts. We still detect a fresh install for sensible
    // first-run defaults (e.g. Windows OpenSSH) and mark the flow complete.
    const isNewInstall = !hasShownWelcomeFlow()
    ...
    if (__WIN32__) {
      ...
      if (useWindowsOpenSSH === undefined) {
        this._setUseWindowsOpenSSH(isNewInstall)
      } else {
  ...
  public getState(): IAppState {
    ...
    return {
      ...
      repositories,
      recentRepositories: this.recentRepositories.slice(
        0,
        this.recentReposDisplayCount
      ),
      recentReposDisplayCount: this.recentReposDisplayCount,
      repoLastAccessed: this.repoLastAccessed,
      ...
      customShells: this.customShells,
  ...
  private async refreshBranchProtectionState(repository: Repository) {
    ...
    if (branchName !== null) {
      ...
      const repoRulesEnabled = useRepoRulesLogic(account, repository)

      const [pushControl, repoRulesResult] = await Promise.all([
        api.fetchPushControl(owner, name, branchName),
        repoRulesEnabled
          ? (async () => {
              const [slimRulesets, branchRules] = await Promise.all([
                api.fetchAllRepoRulesets(owner, name),
                api.fetchRepoRulesForBranch(owner, name, branchName),
              ])

              if (slimRulesets?.length) {
                const calls: Promise<IAPIRepoRuleset | null>[] = []
                for (const id of slimRulesets.map(r => r.id)) {
      ...
      const currentRepoRulesInfo = repoRulesResult ?? new RepoRulesInfo()
  ...
  private updateRecentRepositories(
    ...
    this.repoLastAccessed.set(currentRepositoryId, Date.now())
    this.persistRepoLastAccessed()
    ...
    this.recentRepositories = recentRepositories
    const displaySlice = recentRepositories.slice(
      0,
      this.recentReposDisplayCount
    )
    this.notificationsStore.setRecentRepositories(
      this.repositories.filter(r => displaySlice.includes(r.id))
    )
    this.emitUpdate()
  }

  public _setRecentReposDisplayCount(count: number) {
    this.recentReposDisplayCount = Math.min(Math.max(count, 1), MaxRecentReposDisplayCount)
    setNumber(RecentReposDisplayCountKey, this.recentReposDisplayCount)
    this.emitUpdate()
  }

  private loadRepoLastAccessed() {
    const stored = getObject<Record<string, number>>(RepoLastAccessedKey)
    if (stored) {
      this.repoLastAccessed = new Map(
        Object.entries(stored).map(([k, v]) => [Number(k), v])
      )
    }
  }

  private persistRepoLastAccessed() {
    setObject(
      RepoLastAccessedKey,
      Object.fromEntries(this.repoLastAccessed)
    )
  }
  ...
  public async loadInitialState() {
    ...
    this.loadRepoLastAccessed()
    this.recentReposDisplayCount = Math.min(
      Math.max(
        getNumber(RecentReposDisplayCountKey, DefaultRecentReposDisplayCount),
        1
      ),
      MaxRecentReposDisplayCount
    )
    ...
    this.customEditors =
      getObject<ReadonlyArray<ICustomIntegration>>(customEditorsKey) ?? []
    this.customShells =
      getObject<ReadonlyArray<ICustomIntegration>>(customShellsKey) ?? []
    ...
    return this.withIsCommitting(repository, async () => {
      const result = await gitStore.performFailableOperation(
        async () => {
          ...
          let aborted = false
          // Multi-account: attribute the commit to the active account (the
          // first in the list) so switching accounts switches commit identity.
          const activeAccount = this.accounts.at(0)
          const author = activeAccount
            ? {
                name: activeAccount.name || activeAccount.login,
  ...
  private async refreshAfterCheckout(
    ...
    this.updateCheckoutProgress(repository, {
      kind: 'checkout',
      title: t(__DARWIN__ ? 'Refreshing Repository' : 'Refreshing repository'),
      description: t('Checking out'),
      value: 1,
      target: commitish,
    })
  ...
  public async _deleteBranch(
    ...
    return this.withRefreshedGitHubRepository(repository, async repository => {
      ...
      if (branch.type === BranchType.Remote) {
        ...
        if (remote === undefined) {
          throw new Error(t('Could not determine remote url from: {{ref}}.', { ref: branch.ref }))
        }
  ...
  private getBranchToPush(
    ...
    if (tip.kind === TipState.Unborn) {
      throw new Error(t('The current branch is unborn.'))
    }

    if (tip.kind === TipState.Detached) {
      throw new Error(t('The current repository is in a detached HEAD state.'))
    }
  ...
  private async performPush(
    ...
    return this.withPushPullFetch(repository, async () => {
      ...
      const pushTitle = t('Pushing to {{remoteName}}', { remoteName })
      ...
      await gitStore.performFailableOperation(
        async () => {
          ...
          const refreshTitle = t(__DARWIN__ ? 'Refreshing Repository' : 'Refreshing repository')
          const refreshStartProgress = pushWeight + fetchWeight

          this.updatePushPullFetchProgress(repository, {
            kind: 'generic',
            title: refreshTitle,
            description: t('Fast-forwarding branches'),
  ...
  private async performPull(repository: Repository): Promise<void> {
    return this.withPushPullFetch(repository, async () => {
      ...
      if (!remote) {
        throw new Error(t('The repository has no remotes.'))
      }
      ...
      if (tip.kind === TipState.Valid) {
        ...
        try {
          ...
          await Promise.all([
            mergeBase
              ? gitStore.reconcileHistory(mergeBase)
              : Promise.resolve(),
            this.refreshBranchProtectionState(repository),
            this._refreshRepository(repository),
          ])
  ...
  public async _deleteWorktree(
    ...
    if (isDeletingCurrentWorktree) {
      ...
      if (main === undefined) {
        throw new Error(t('Could not find main worktree'))
      }
  ...
  public async _startCopilotConflictResolution(
    ...
    try {
      ...
      if (result === null) {
        throw new Error(t('Copilot conflict resolution returned no results'))
      }
  ...
  /** Switch the active account (drives auth, API, and commit identity). */
  public async _setActiveAccount(account: Account): Promise<void> {
    log.info(`[AppStore] switching active account to ${account.login}`)
    await this.accountsStore.setActiveAccount(account)
  }
    ...
    try {
      if (moveToTrash) {
        try {
          ...
          this.emitError(
            new Error(
              t('Failed to move the repository directory to {{trashLabel}}.\n\nA common reason for this is that the directory or one of its files is open in another program.', { trashLabel: TrashNameLabel() })
            )
          )
          return
        }
  ...
  private getInvalidRepoPathsMessage(
    ...
    if (invalidPaths.length === 1) {
      return t("{{path}} isn't a Git repository.", { path: invalidPaths[0] })
    }

    const listed = invalidPaths
    ...
    const extra =
      invalidPaths.length > MaxInvalidFoldersToDisplay
        ...
        : ''
    return t("The following paths aren't Git repositories:\n\n{{listed}}{{extra}}", {
      listed,
      extra,
    })
  }
  ...
  public _setCustomEditors(editors: ReadonlyArray<ICustomIntegration>) {
    this.customEditors = editors
    setObject(customEditorsKey, editors)
    this.emitUpdate()
  }

  public _setCustomShells(shells: ReadonlyArray<ICustomIntegration>) {
    this.customShells = shells
    setObject(customShellsKey, shells)
    this.emitUpdate()
  }

  public async _openInCustomEditor(path: string, editor: ICustomIntegration) {
    try {
      if (!editor.path && editor.name) {
        const match = await findEditorOrDefault(editor.name)
        if (match) {
          await launchExternalEditor(path, match)
          return
        }
      }
      await launchCustomExternalEditor(path, editor)
    } catch (error) {
      this.emitError(
        new ExternalEditorError(error as string, { openPreferences: true })
      )
    }
  }

  public async _openInCustomShell(path: string, shell: ICustomIntegration) {
    try {
      if (!shell.path && shell.name) {
        const match = await findShellOrDefault(shell.name as any)
        if (match) {
          await launchShell(match, path, error => this._pushError(error))
          return
        }
      }
      await launchCustomShell(shell, path, error => {
        this.emitError(
          new ExternalEditorError(error, { openPreferences: true })
        )
      })
    } catch (error) {
      this.emitError(
        new ExternalEditorError((error as Error).message, {
  ...
  private async resolveCopilotModelRequest(
    ...
    if (provider.authKind !== 'none') {
      try {
        ...
        throw new Error(
          t("Could not read the credential for the custom Copilot provider '{{name}}' from the OS keychain: {{message}}", { name: provider.name, message })
        )
      }
    }

    if (provider.authKind !== 'none' && (secret === null || secret === '')) {
      const credKind = provider.authKind === 'bearer' ? t('bearer token') : t('API key')
      throw new Error(
        t("No {{credKind}} is stored for the custom Copilot provider '{{name}}'. Open Settings → Copilot → Providers and re-enter the credential.", { credKind, name: provider.name })
      )
    }
