  ...
  public async resolveOAuthRequest(action: IOAuthAction) {
    ...
    if (token) {
      ...
      this.state.oauthState.onAuthError(
        new Error(t('Failed retrieving authenticated user'))
      )
    }
  }
  ...
  public async setEndpoint(url: string): Promise<void> {
    ...
    try {
      ...
      if (e.name === InvalidURLErrorName) {
        error = new Error(
          t("The GitHub Enterprise instance address doesn't appear to be a valid URL. We're expecting something like https://example.ghe.com.")
        )
      } else if (e.name === InvalidProtocolErrorName) {
        error = new Error(
          t('Unsupported protocol. Only https is supported when authenticating with GitHub Enterprise instances.')
        )
      }
