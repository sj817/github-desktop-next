...
export class NotificationsStore {
  ...
  private async handlePullRequestCommentEvent(
    ...
    const title = t('@{{login}} commented on your pull request', { login: comment.user.login })
  ...
  private async handlePullRequestReviewSubmitEvent(
    ...
    const title = t('@{{login}} {{reviewVerb}} your pull request', { login: review.user.login, reviewVerb })
    const body = `${pullRequest.title} #${
      pullRequest.pullRequestNumber
  ...
  private async handleChecksFailedEvent(
    ...
    const title = t('Pull Request checks failed')
    const body =
      numberOfFailedChecks === 1
        ? t('{{prTitle}} #{{prNumber}} ({{sha}})\n{{count}} check was not successful.', {
            prTitle: pullRequest.title,
            prNumber: pullRequest.pullRequestNumber,
            sha: shortSHA,
            count: numberOfFailedChecks,
          })
        : t('{{prTitle}} #{{prNumber}} ({{sha}})\n{{count}} checks were not successful.', {
            prTitle: pullRequest.title,
            prNumber: pullRequest.pullRequestNumber,
            sha: shortSHA,
            count: numberOfFailedChecks,
