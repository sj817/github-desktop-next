...
type TimeUnitDescriptor = {
  shortUnit: string
  longUnit: string
  longUnitPlural: string
  ms: number
}

const units: TimeUnitDescriptor[] = [
  { shortUnit: 'd', longUnit: 'day', longUnitPlural: 'days', ms: 86400000 },
  { shortUnit: 'h', longUnit: 'hour', longUnitPlural: 'hours', ms: 3600000 },
  { shortUnit: 'm', longUnit: 'minute', longUnitPlural: 'minutes', ms: 60000 },
  { shortUnit: 's', longUnit: 'second', longUnitPlural: 'seconds', ms: 1000 },
]
...
export const formatLongPreciseDuration = (ms: number) => {
  ...
  for (const unit of units) {
    if (parts.length > 0 || ms >= unit.ms || unit.shortUnit === 's') {
      const qty = Math.floor(ms / unit.ms)
      ms -= qty * unit.ms
      const unitName = qty === 1 ? t(unit.longUnit) : t(unit.longUnitPlural)
      parts.push(`${qty} ${unitName}`)
    }
  }
