...
function parseGitHubReleaseBody(body: string): ReadonlyArray<string> {
  return body
    .split('\n')
...
export async function getChangeLog(
  ...
  const releasesURL =
    'https://api.github.com/repos/sj817/github-desktop-next/releases'
  ...
  try {
    ...
    const ghReleases: ReadonlyArray<IGitHubRelease> = await response.json()
    ...
    return ghReleases
      ...
      .map(r => ({
        ...
        notes: parseGitHubReleaseBody(r.body || ''),
        pub_date: r.published_at,
        version: r.tag_name.replace(/^v/, ''),
      }))
  } catch (e) {
    log.error('Error fetching changelog from GitHub Releases', e)
    return []
  }
}
